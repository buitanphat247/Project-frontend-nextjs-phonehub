...
interface UseSmartwatchesOptions {
  productsPerPage?: number
}

// Category ID mapping - smartwatches category ID
const SMARTWATCHES_CATEGORY_ID = 5 // Smartwatches category ID từ API
...
export function useSmartwatches({ productsPerPage = 12 }: UseSmartwatchesOptions = {}) {
  ...
  const updateUrlParams = useCallback((page: number, search: string, brand: string, priceRange: string) => {
    ...
    const newUrl = `/smartwatches${queryString ? `?${queryString}` : ''}`
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const response = await getBrandsByCategory(SMARTWATCHES_CATEGORY_ID)
  ...
  useEffect(() => {
    ...
    if (!window.location.pathname.startsWith('/smartwatches')) return
  ...
  useEffect(() => {
    ...
    if (lastPathname && lastPathname !== currentPathname) {
      const isCategoryChange = 
        ...
        (lastPathname.startsWith('/smartwatches') && !currentPathname.startsWith('/smartwatches'))

      if (isCategoryChange && currentPathname.startsWith('/smartwatches')) {
        ...
        setCurrentPage(1)
        // Clear URL params
        router.push('/smartwatches', { scroll: false })
      }
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (hasParams && window.location.pathname.startsWith('/smartwatches')) {
        router.push('/smartwatches', { scroll: false })
      }
    }
  }, [router])
