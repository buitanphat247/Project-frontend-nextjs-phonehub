...
interface UseLaptopsOptions {
  productsPerPage?: number
}

// Category ID mapping - laptops category ID
const LAPTOPS_CATEGORY_ID = 3 // Laptop category ID từ API
...
export function useLaptops({ productsPerPage = 12 }: UseLaptopsOptions = {}) {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const response = await getBrandsByCategory(LAPTOPS_CATEGORY_ID)
  ...
  useEffect(() => {
    ...
    if (lastPathname && lastPathname !== currentPathname) {
      ...
      if (isCategoryChange && currentPathname.startsWith('/laptops')) {
        ...
        setCurrentPage(1)
        // Clear URL params
        router.push('/laptops', { scroll: false })
      }
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (hasParams && window.location.pathname.startsWith('/laptops')) {
        router.push('/laptops', { scroll: false })
      }
    }
  }, [router])
