...
// Category ID mapping - phones category ID
const PHONES_CATEGORY_ID = 2 // Điện thoại category ID từ API
...
export function usePhones({ productsPerPage = 12 }: UsePhonesOptions = {}) {
  ...
  const updateUrlParams = useCallback((page: number, search: string, brand: string, priceRange: string) => {
    ...
    const newUrl = `/phones${queryString ? `?${queryString}` : ''}`
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const response = await getBrandsByCategory(PHONES_CATEGORY_ID)
  ...
  useEffect(() => {
    ...
    if (!window.location.pathname.startsWith('/phones')) return
  ...
  useEffect(() => {
    ...
    if (lastPathname && lastPathname !== currentPathname) {
      ...
      if (isCategoryChange && currentPathname.startsWith('/phones')) {
        ...
        setCurrentPage(1)
        // Clear URL params
        router.push('/phones', { scroll: false })
      }
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (hasParams && window.location.pathname.startsWith('/phones')) {
        router.push('/phones', { scroll: false })
      }
    }
  }, [router])
